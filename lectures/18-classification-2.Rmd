...
```{r}
newdata <- as.data.frame(select(train_dat, -arr_delay_status))
pred_treebag <- predict(treebagfit, newdata)
confusionMatrix(train_dat$arr_delay_status, pred_treebag)
```
...
```{r}
newdata <- as.data.frame(select(train_dat, -arr_delay_status))
pred_rf <- predict(rffit, newdata)
confusionMatrix(train_dat$arr_delay_status, pred_rf)
```
...
```{r}
newdata <- as.data.frame(select(train_dat, -arr_delay_status))
pred_boost <- predict(boostfit, newdata)
confusionMatrix(train_dat$arr_delay_status, pred_boost)
```
