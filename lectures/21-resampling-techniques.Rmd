...
the true proportion ($p$) is $p=0.47$. The following code simulates a 
distribution of $\hat{p}$ after 85 pollsters all conducted surveys of 
sample sizes $n=1000$. 
...
This opinion poll reported voting intention proportions of 48.5% for the 
...
abline(v = 0.50, col = "red", lty = 5)
```

What does this tell us?

This is the sampling distribution of our statistic $\hat{p}$, assuming 
a true $p$ of 0.485. Assuming a true $p$ of 0.485, we can then calculate 
the expected proportion of observed $\hat{p}$ above 0.5, given our 
assumptions (e.g. sample sizes).

But what if we don't know the true value $p$? This is the more common 
situation in practice.

For an election, we are also more interested in the final voting result 
...
opinion poll is all that we have access to. In statistical inferenxce, 
it is crucial to think about what exactly is the population you are 
sampling from.
...
This is a major advantage of the bootstrap -- we can estimates 
...
for (i in 1:B) {
  sample_ilec <- sample(time_ilec, n_ilec, replace = TRUE)
  sample_clec <- sample(time_clec, n_clec, replace = TRUE)
  time_ilec_boot[i] <- mean(sample_ilec, trim = 0.25)
  time_clec_boot[i] <- mean(sample_clec, trim = 0.25)
